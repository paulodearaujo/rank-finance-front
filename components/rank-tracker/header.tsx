					...
					>
						...
						>
							<span className="inline-flex items-center gap-2">
								<IconBrandApple className="h-4 w-4" aria-hidden="true" /> App
								Store
							</span>
						</DropdownMenuCheckboxItem>
						<DropdownMenuCheckboxItem
						...
						>
							<span className="inline-flex items-center gap-2">
								<IconBrandGooglePlay className="h-4 w-4" aria-hidden="true" />{" "}
								Google Play
							</span>
						</DropdownMenuCheckboxItem>
					</DropdownMenuContent>
				...
				<Select value={beforeRunId || ""} onValueChange={setBeforeRunId}>
					...
					>
						<SelectValue placeholder="Before">
							{formatRunDate(
								availableRuns.find((r) => r.run_id === beforeRunId),
							)}
						</SelectValue>
					</SelectTrigger>
					<SelectContent>
				...
				<Select value={afterRunId || ""} onValueChange={setAfterRunId}>
					...
					>
						<SelectValue placeholder="After">
							{formatRunDate(
								availableRuns.find((r) => r.run_id === afterRunId),
							)}
						</SelectValue>
					</SelectTrigger>
					<SelectContent>
						{availableRuns.map((run, index) => (
							<SelectItem
								...
								disabled={
									index >
									availableRuns.findIndex((r) => r.run_id === beforeRunId)
								}
							>
								{formatRunDate(run)}
							</SelectItem>
