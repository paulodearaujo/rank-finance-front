...
import { InitialLoadingBlur } from "@/components/initial-loading-blur";
...
export default async function RankTrackerPage({ searchParams }: RankTrackerPageProps) {
  ...
  if (availableRuns.length === 0) {
    return (
      ...
      >
        <div id="empty-state-container" className="container mx-auto flex-1">
          <RankTrackerEmpty />
        </div>
        <SiteFooter />
      </div>
  ...
  return (
    <>
      {/* Immediate loading blur - rendered server-side */}
      <div
        id="initial-loading-blur"
      ...
      {/* Client component to remove blur when ready */}
      <InitialLoadingBlur />
      ...
      >
        ...
        <main id="rank-tracker-content" className="container mx-auto py-6 relative flex-1">
